...
const RAW_PRODUCTS = [
  { id: 1, name: '138 酒水套餐', price: 138, bonusPoints: 3500,  cloudImage: CLOUD_BASE + 'combo_138.png', desc: '基础饮品畅饮' },
  { id: 2, name: '218 酒水套餐', price: 218, bonusPoints: 7100,  cloudImage: CLOUD_BASE + 'combo_218.jpg', desc: '基础饮品+鸡尾酒畅饮' },
  { id: 3, name: '398 酒水套餐', price: 398, bonusPoints: 15000, cloudImage: CLOUD_BASE + 'combo_398.jpg', desc: '基础饮品+特调鸡尾酒畅饮' },
  { id: 4, name: '99 早鸟票 - 20点前到店可使用', price: 99, bonusPoints: 3000, cloudImage: CLOUD_BASE + 'special_99.jpg', desc: '晚8点前到店可用，基础饮品畅饮' },
  { id: 5, name: '89 下午茶特惠 - 13-17点可用', price: 89, bonusPoints: 3000, cloudImage: CLOUD_BASE + 'special_99.jpg', desc: '下午茶特惠，13-17点间可用，茶水畅饮+精美点心1份' },
]
...
Page({
  ...
  async _payWithBalance(product, balanceDeduct) {
    ...
    if (res.result.code === 0) {
      this.setData({ showPayModal: false, selectedProduct: null })
      const gp = res.result.giftPoints || 0
      wx.showToast({ title: gp > 0 ? `购买成功 +${gp}积分` : '购买成功', icon: gp > 0 ? 'none' : 'success' })
    } else {
  ...
  async _payMixed(product, balanceDeduct, wechatAmount) {
    ...
    this.setData({ showPayModal: false, selectedProduct: null })
    const gp = product.bonusPoints || 0
    wx.showToast({ title: gp > 0 ? `支付成功 +${gp}积分` : '支付成功', icon: gp > 0 ? 'none' : 'success' })
  },
})
